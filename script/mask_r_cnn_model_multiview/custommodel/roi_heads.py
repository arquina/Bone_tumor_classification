...
def fastrcnn_loss(class_logits, box_regression, labels, regression_targets):
    ...
    # advanced indexing
    batch_size = int(class_logits.shape[0]/ 512)
    num = 512
    total_index = []
    for index in range(batch_size):
        partial_class_logits = class_logits[index * num : (index+1)*num,:]
        partial_labels = labels[index*num:(index+1)*num]
        partial_loss = F.cross_entropy(partial_class_logits, partial_labels, reduction = 'none')
        partial_index = torch.topk(partial_loss, 1, largest = False, sorted = True)[1]
        partial_index = partial_index + index * num
        if len(total_index) == 0:
            total_index = partial_index
        else:
            total_index =torch.cat([total_index, partial_index])
    ...
    return classification_loss, box_loss, index
...
class RoIHeads(nn.Module):
    ...
    def postprocess_detections(self,
        ...
        for boxes, scores, image_shape in zip(pred_boxes_list, pred_scores_list, image_shapes):
            ...
            box_features = torch.cat([box_features, box_features, box_features], dim = 0)
            index = torch.topk(scores, 1, largest=True, sorted=True)[1]
            box_features = box_features[index]
        ...
        return all_boxes, all_scores, all_labels, box_features
    ...
    def forward(self,
        ...
        if self.training:
            ...
            loss_classifier, loss_box_reg, index = fastrcnn_loss(
            ...
            for idx in range(batch_size):
                temp_features = box_features[index[idx:(idx + 1)]].unsqueeze(0)
                if len(selected_features) == 0:
                    selected_features = temp_features
                else:
                    selected_features = torch.cat([selected_features, temp_features], dim = 0)

        else:
            boxes, scores, labels, box_features = self.postprocess_detections(class_logits, box_regression, proposals, image_shapes, box_features)
            ...
            for i in range(num_images):
                ...
                selected_features = box_features
        if self.has_mask():
        ...
        return result, losses, selected_features
